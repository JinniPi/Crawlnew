...
class crawlthoisu(scrapy.Spider):

	name = "thoisu"
	allowed_domains = ['vnexpress.net']
	start_urls = ['https://vnexpress.net/tin-tuc/thoi-su']
	count = 0
	sets = set()
	items = []
	...
	def parse(self, response):
		...
		for url in urls:
			...
			if url in self.sets or re.search("/thoi-su/",url) == None or re.search("/infographics/",url) != None :
				continue
			else:
				self.sets.add(url)
