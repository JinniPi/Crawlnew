...
class crawlkinhdoanh(scrapy.Spider):

	name = "kinhdoanh"
	allowed_domains = ['vnexpress.net']
	start_urls = ['https://kinhdoanh.vnexpress.net/']
	count = 0
	sets = set()
	items = []
	...
	def parse(self, response):
		...
		for url in urls:
			...
			if url in self.sets or re.search("/infographics/",url) != None or re.search("/photo/",url) != None :
				continue
			else:
				self.sets.add(url)
