...
class crawlnewSpider(scrapy.Spider):

	name = "trang"
	allowed_domains = ['vnexpress.net']
	start_urls = ['https://vnexpress.net/tin-tuc/phap-luat']
	count = 0
	sets = set()
	items = []
	...
	def parse(self, response):
		...
		for url in urls:
			...
			if url in self.sets or re.search("/video/",url) != None or re.search("/tu-van/",url) != None or re.search("/ho-so-pha-an/",url) != None :
				continue
			else:
				self.sets.add(url)
